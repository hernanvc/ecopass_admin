...
class EventPays extends Component {
    constructor(props) {
        super(props)
        this.state = {
            active: 5,
    ...
    async componentDidMount() {
        ...
        let fetch = await requestHttp.requestGet('ticketPay/event/' + id)
        if (fetch) {
            console.log(fetch)
            await this.setState({
    ...
    async deleteTicket(e) {
        ...
        let putTicket = await requestHttp.deleteItem('ticketPay/'+ e.id )
        if (putTicket) {
            this.setState({
                loader: false,
            })
